...
export class Auth {
  ...
  async getJWTToken(): Promise<string | null> {
    ...
    const tokenCookie = cookies.find(
      (cookie) =>
        cookie.name ===
        'eyJhbGciOiJkaXIiLCJlbmMiOiJBMjU2R0NNIn0..IH5rWcYyqHiLDGgg.3qxpxUET06Xeg6uW2_LoBOf2rv1ZvLQLPJbuj2ZkkCTk5O9k3yT9ukbNXU0YEivIpbq_wGq5TrkFnQGRHvzmv82liV0pN1ug78Zu_vH6BQz6unHMge4KdD8FA_74fxwKMILyRbFJ_54bvEdRgR5gx9m-nGnzCmTzr3faYpTxf0-_sRZvwi1VVGQXOf4lVwe1m0HqKbENkUu3GC_AS8i5SRgQx07fOS5xPJUMFzkYm3xkPF9pbV1Lbzr9T7'
    )
    return tokenCookie ? tokenCookie.value : null
  }

  async acceptValidJWTToken(): Promise<string | null> {
    const cookies = await this.page.context().cookies()
    const tokenCookie = cookies.find((cookie) => cookie.name === 'getJWTToken')
    return tokenCookie ? tokenCookie.value : null
  }
